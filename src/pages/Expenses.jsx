...
export default function Expenses({ user, data, symbol }) {
  const s = symbol || '₱'
  const [form, setForm] = useState({ desc: '', amount: '', date: today(), cat: 'Food & Dining', recur: '', accountId: data.accounts?.[0]?._id || '' })
  ...
  async function handleAdd() {
    ...
    const err = validateAmount(form.amount); if (err) return alert(err); await fsAddTransaction(user.uid, 'expenses', { ...form, amount: parseFloat(form.amount), type: 'expense', accountBalanceLinked: Boolean(form.accountId) }, data.accounts)
